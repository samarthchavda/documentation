...
configuring a product in the Odoo ecosystem: Goods or Services.
...
- :guilabel:`Product`: Limit the default delay costs setting to one product from the field’s
  drop-down list. Leave the Product field blank if the defaults apply to all products.

If the **Inventory** app is installed, the *Default Padding Time* section displays. The time
inputted into :guilabel:`Padding` field blocks a rental product from being available for
...
.. note::
   ...
   Customer/Rental upon rental, and back upon return). **Do not** modify to avoid corrupting
   inventory tracking.
...
All the selected options appear as Kanban cards. For Goods, the card displays the name, rental rate,
and amount on hand. For Services, the card displays the name, the number of variants if configured,
