...
Selecting :guilabel:`Import records` opens a new page with a :guilabel:`Import Template for
...
- Columns that are unnecessary may be removed.
...
When importing a completed template, Odoo displays a page that shows all the elements of the newly
...
:guilabel:`Import`, and Odoo returns to the :guilabel:`Products` page. The new information on can be
...
- :guilabel:`Variant Creation Mode`: The way variants are created when applied to a product. This
  option **cannot** be changed once the attribute has been changed on a least one product. There are
  three variant creation mode options:
