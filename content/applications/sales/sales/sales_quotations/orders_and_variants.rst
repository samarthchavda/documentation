...
To enable that setting, go to the :menuselection:`Sales app --> Configuration --> Settings`, and
scroll to the *Product Catalog* section. Then, check the box next to the :guilabel:`Variant Grid
...
At the bottom of the :guilabel:`Attributes \& Variants` tab, there is a *Sales Variant Selection*
section with two options: :guilabel:`Product Configurator` and :guilabel:`Order Grid Entry`.
...
The product configurator feature appears as a *Configure your product* pop-up window, as soon as a
...
.. note::
   This *Configure your product* pop-up window also appears if the :guilabel:`Order Grid Entry`
   setting is **not** activated, as it is the default option Odoo uses when dealing with product
   variants on quotations and/or sales orders.
...
The order grid entry feature appears as a *Choose Product Variants* pop-up window, as soon as a
product with (at least two) variants is added to a quotation or sales order, but **only** if the
:guilabel:`Order Grid Entry` option is selected on its product form.
...
to add to the quotation/sales order at once.
