...
Work orders can be planned directly from the work order form.

Navigate to :menuselection:`Manufacturing app --> Operations --> Work Orders`, then open a work
order from the list.
...
show all work orders. The *Planning* view shows a timeline of all the work orders scheduled for each
operation.

By default, the *Planning* and *Kanban* views are grouped by *Work Center*. This grouping is best
used to view work orders grouped by the work center at which they take place. This view can help
identify overloaded machines or bottlenecks in production.
...
- :guilabel:`Planning Issues`: Shows work orders that have planning issues, such as being scheduled
  out of sync with its work order dependencies.
...
Alternatively, on the side of the screen, a list of unplanned work orders appears, with a title that
reads :guilabel:`# to schedule`.
...
Assign a work order to an employee by clicking the :icon:`fa-user-plus` :guilabel:`(Assign)` icon in
the work order card, then selecting an employee from the list, or click the :icon:`fa-pencil`
...
Update the status of a work order by clicking the circle in the bottom corner and selecting a
status.
