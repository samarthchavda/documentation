...
dashboard. In the :guilabel:`Search` field, search for `USPS`.
...
.. important::
   Enabling the USPS shipping connector automatically creates two default delivery methods:
...
The options in the *USPS Configuration* tab of a USPS delivery method form are used to connect the
...
- :guilabel:`CRID`: The CRID is a number that identifies a physical business address across all
  USPS systems and applications.
- :guilabel:`MID`: This is listed as the :guilabel:`Master Mailer ID` in the COP.
- :guilabel:`Manifest MID`: This is listed as the :guilabel:`Label Mailer ID` in the COP.
...
- :guilabel:`Delivery Nature`: Specify whether the delivery method is :guilabel:`Domestic` or
...
- :guilabel:`Unpublished` / :guilabel:`Published`: Determines if this delivery method is available
