...
<../../finance/accounting/customer_invoices/cash_rounding>` is supported by using the
:guilabel:`Add a rounding line` rounding method. This adds a rounding line that changes the paid
amount but does not change the amount in the |CFDI|.
...
#. The substituted invoice requests the cancellation automatically when the new one is signed.
...
.. note::
   ...
   field. If the new payment is signed after the substituted document is linked the older payment is
   then cancelled.
...
on the invoice and indicate the relation. There are seven possible values:
...
In certain occasions, such as when creating invoices in another software or in the |SAT| directly,
it is possible to upload the invoices in Odoo. The XML Reader can retrieve the data from an XML
file. To do this, navigate to :menuselection:`Accounting --> Customers --> Invoices` and, in the
list view, click the :guilabel:`Upload` button to select any number of XML files. Draft invoices are
automatically created. This can also work by dragging the files from a local storage and dropping
them in the view.
...
canceled at any time. If the :guilabel:`Print` button is used, the PDF document includes all the
...
Due to the account hierarchy on :ref:`parent accounts <accounting/account-groups>`, set the
appropriate :guilabel:`Parent Account` on each account to create a hierarchy for reports. Accounts
without a parent are level 1 accounts; accounts below them are level 2, and so on.

.. note::
   - Active accounts must be longer than five digits, or they trigger errors in the report. By
     default, Odoo marks the accounts in yellow if the numbering could cause issues later on.
...
The trial balance reports the initial balance, credit, and total balance of the accounts, provided
...
Trial Balance`, select the date, and click the :icon:`fa-cog` :guilabel:`(action menu)`. Then,
select :guilabel:`SAT (XML)` and select the appropriate :guilabel:`Submit Type` from the wizard,
depending on whether the trial balance is `Normal` or `Supplementary`.
...
A-29 form. In Odoo, it is possible to download the records of all transactions as a TXT file. This
file can be uploaded to the form, reducing the data that is manually added.
...
In case new taxes are created on the database, there are two types of tags. Tags containing **tax**
...
There are two reports specific to the Mexican localization: Balance Sheet and Profit and Loss.
...
information, including the *Usage* and *Fiscal Regime*, once they enter the five-digit code provided
on the receipt.
...
.. note::
   ...
   shipment leaves the warehouse. The :guilabel:`Delivery Date` is the time that the shipment is
